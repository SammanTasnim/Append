...
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the proshoot_breadcrumb class,
 * which should be used in theme templates.
 *
 * @since  0.1.0
 * @access public
 * @param  array $args Arguments to pass to proshoot_breadcrumb.
 * @return void
 */
function append_breadcrumb( $args = array() ) {

  $breadcrumb = apply_filters( 'proshoot_breadcrumb_object', null, $args );

  if ( ! is_object( $breadcrumb ) ) {

    $breadcrumb = new append_breadcrumb( $args );
  }
...
class append_breadcrumb {
  ...
  /**
   * Sets up the breadcrumb trail properties.  Calls the `proshoot_breadcrumb::add_items()` method
   ...
   *     @type array     $labels         Text labels. @see proshoot_breadcrumb::set_labels()
   *     @type array     $post_taxonomy  Taxonomies to use for post types. @see proshoot_breadcrumb::set_post_taxonomy()
  ...
  public function __construct( $args = array() ) {
    ...
    $this->args = apply_filters( 'proshoot_breadcrumb_args', wp_parse_args( $args, $defaults ) );
  ...
  public function trail() {
    ...
    if ( 0 < $item_count ) {
      ...
      $breadcrumb = sprintf(
        '<%1$s role="navigation" aria-label="%2$s" class="proshoot breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
    ...
    $breadcrumb = apply_filters( 'proshoot_breadcrumb', $breadcrumb, $this->args );
  ...
  protected function set_labels() {
    ...
    $defaults = array(
      'browse'              => esc_html__( 'Browse:',                               'proshoot' ),
      'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'proshoot' ),
      'home'                => esc_html__( 'Home',                                  'proshoot' ),
      'error_404'           => esc_html__( '404 Not Found',                         'proshoot' ),
      'archives'            => esc_html__( 'Archives',                              'proshoot' ),
      // Translators: %s is the search query.
      'search'              => esc_html__( 'Search results for: %s',                'proshoot' ),
      // Translators: %s is the page number.
      'paged'               => esc_html__( 'Page %s',                               'proshoot' ),
      // Translators: %s is the page number.
      'paged_comments'      => esc_html__( 'Comment Page %s',                       'proshoot' ),
      // Translators: Minute archive title. %s is the minute time format.
      'archive_minute'      => esc_html__( 'Minute %s',                             'proshoot' ),
      // Translators: Weekly archive title. %s is the week date format.
      'archive_week'        => esc_html__( 'Week %s',                               'proshoot' ),
    ...
    $this->labels = apply_filters( 'proshoot_breadcrumb_labels', wp_parse_args( $this->args['labels'], $defaults ) );
  }
  ...
  protected function set_post_taxonomy() {
    ...
    $this->post_taxonomy = apply_filters( 'proshoot_breadcrumb_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
  }
  ...
  protected function add_items() {
    ...
    $this->items = array_unique( apply_filters( 'proshoot_breadcrumb_items', $this->items, $this->args ) );
  }
  ...
  protected function add_minute_hour_archive_items() {
    ...
    if ( true === $this->args['show_title'] )
      $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'proshoot' ) ) );
  }
  ...
  protected function add_minute_archive_items() {
    ...
    // Add the minute item.
    if ( true === $this->args['show_title'] )
      $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'proshoot' ) ) );
  }
  ...
  protected function add_hour_archive_items() {
    ...
    // Add the hour item.
    if ( true === $this->args['show_title'] )
      $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'proshoot' ) ) );
  }
  ...
  protected function add_day_archive_items() {
    ...
    $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'proshoot' ) ) );
    $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'proshoot' ) ) );
    $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'proshoot' ) ) );
  ...
  protected function add_week_archive_items() {
    ...
    $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'proshoot' ) ) );
    $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'proshoot' ) ) );
  ...
  protected function map_rewrite_tags( $post_id, $path ) {
    ...
    if ( is_array( $matches ) ) {
      ...
      foreach ( $matches as $match ) {
        ...
        if ( '%year%' == $tag )
          $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'proshoot' ) ) ) );

        // If using the %monthnum% tag, add a link to the monthly archive.
        elseif ( '%monthnum%' == $tag )
          $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'proshoot' ) ) ) );

        // If using the %day% tag, add a link to the daily archive.
        elseif ( '%day%' == $tag )
          $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'append' ) ) ) );
